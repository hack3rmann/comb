...
auto test_parse_sequence() -> void {
    auto const result1 = prefix<char>("hello").parse("hello, world!");

    comb_assert(result1.ok());
    comb_assert_eq(result1.get_value(), "hello");
    comb_assert_eq(result1.tail, ", world!");

    auto const result2 = prefix<char>("Minecraft").parse("Hello, World!");
...
auto test_parse_combine() -> void {
    ...
    auto const result1 =
        (prefix<char>("Terraria") | prefix<char>("Minecraft")).parse(src);
    ...
    auto const result2 =
        (prefix<char>("Minecraft") | prefix<char>("Terraria")).parse(src);
    ...
    auto const result3 =
        (prefix<char>("Terraria") | prefix<char>("VintageStory")).parse(src);
